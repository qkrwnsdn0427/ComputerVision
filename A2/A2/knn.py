...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##a^2+b^2=c^2 => ((x2-x1)^2+(y2-y1)^2).sqrt
    for i in range(num_train):
        for j in range(num_test):
            train=x_train[i].flatten()
            test=x_test[j].flatten()
            dists[i,j]=torch.sum((train-test)**2).sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    for i in range(num_train):
        dists[i]=(((x_test-x_train[i])**2).sum(dim=(1,2,3))).sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_sum=torch.sum(train_flat**2,dim=1,keepdim=True)
    test_sum=torch.sum(test_flat**2,dim=1,keepdim=True)
    test_sum=test_sum.t()
    ## (x-y)^2=x^2+y^2-2xy
    dists=train_sum+test_sum-2*torch.matmul(train_flat,test_flat.t())
    dists=dists.sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(idx.shape[1]):
        a,index=torch.max(y_train[idx[:,i]].bincount(),dim=0)
        y_pred[i]=index
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for numfold in range(num_folds):
        x_copy=list(x_train_folds)
        y_copy=list(y_train_folds)
        x_test=x_copy.pop(numfold)
        y_test=y_copy.pop(numfold)
        x_train=torch.cat(x_copy, dim=0)
        y_train=torch.cat( y_copy, dim=0)
        classifier=KnnClassifier(x_train,y_train)
        acc.append(classifier.check_accuracy(x_test,y_test,k))
