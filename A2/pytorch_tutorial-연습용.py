...
def hello():
    ...
    """
    print('Hello from pytorch_tutorial.py!')
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y=torch.tensor([x[1,0],x[0,1],x[3,2]],dtype=x.dtype)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    min,i=torch.min(x,dim=1,keepdim=True)
    y=torch.where(x==min,torch.zeros_like(x),x)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    total=torch.tensor([x.sum() for x in xs])
    y=total/ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    #uniques,inverse_idx=torch.unique(x,sorted=True,return_inverse=True)
    #a,indices=torch.unique(inverse_idx,return_index=True)
    sort_val,sort_idx=torch.sort(x)
    uniq=torch.cat([sort_val[1:]!=sort_val[:-1],torch.tensor([True],device=x.device)])
    uniques=sort_val[uniq]
    indices=sort_idx[uniq]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
