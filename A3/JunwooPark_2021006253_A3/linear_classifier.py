...
def svm_loss_vectorized(
    ...
    scores=torch.mm(X,W) # scores: 128classes x 10train
    N, C=X.shape[0], W.shape[1] # N = 128, C = 10
    correct_score=scores[range(N),y].reshape(-1,1) # 128 x 1
    zeros=torch.zeros_like(scores)
    margin=torch.maximum(scores-correct_score+1,zeros)
    margin[range(N),y]=0
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates=[0.001,0.001,0.02,0.05,0.01]
    regularization_strengths=[0.01,0.01,0.1,1,3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    cls.train(X_train, y_train,lr,reg,num_iters,200,False) # train
...
def softmax_loss_naive(
    ...
    for i in range(N):
      ...
      loss+=(torch.log(exp_sum)-scores[y[i]])
      for j in range(C):
        if j==y[i]:
          dW[:,j]+=(torch.exp(scores[j])/exp_sum*X[i]-X[i])
        else:
...
def softmax_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    scores=torch.mm(X, W)
    scores_max, a=torch.max(scores,dim=1)

    scores-=scores_max.reshape(-1,1)
    N=X.shape[0]
    correct_score=scores[range(N),y]
    exp_sum=torch.sum(torch.exp(scores),dim=1)
    loss=torch.log(exp_sum)-correct_score
    loss=loss.sum()
    ...
    # 기울기 계산
    minus=torch.zeros_like(scores)
    minus[range(N),y]=-1
    
    dW+=minus.t().mm(X).t()
    dW+=(torch.exp(scores)/exp_sum.reshape(-1,1)).t().mm(X).t()
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates=[0.001,0.01,0.05,0.02,0.01]
