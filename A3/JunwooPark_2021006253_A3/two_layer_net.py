...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    max_valscore=scores.max(dim=1).values # [5,]
    scores-=max_valscore.view(-1,1)
    exp_scores=torch.exp(scores)
    exp_sum=torch.sum(exp_scores,dim=1)
    softmax_func=exp_scores/exp_sum.view(-1,1)
    loss=-torch.sum(torch.log(softmax_func[torch.arange(N),y]))
    loss/=N 
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    grad_scores=softmax_func
    grad_scores[torch.arange(N),y]-=1
    grad_scores/=N

    # backpropagation
    # h1: N x H, grad_scores: N x C
...
def nn_get_search_params():
    ...
    regularization_strengths=[0,0.1,0.001,0.00001]
