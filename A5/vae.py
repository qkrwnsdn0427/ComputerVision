...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        encod_out=self.encoder(x)
        mu=self.mu_layer(encod_out)
        logvar=self.logvar_layer(encod_out)
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        enc_in=torch.cat((x_flatten,c),dim=1)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    kldiv=1+logvar-mu**2-torch.exp(logvar)
    kldiv=-0.5*kldiv.sum()
    loss=rec_term + kldiv
    loss/=N
    ###############################################################################
