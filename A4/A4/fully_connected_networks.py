...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        # relu_out,relu_back=Linear_ReLU.forward(X,self.params['W1'],self.params['b1'])
        # scores,lin_back=Linear.forward(relu_out,self.params['W2'],self.params['b2'])
        ...
        # dx,dw,db=Linear.backward(dout,lin_back)
        # grads['W2']=dw+2*self.params['W2']*self.reg
        # grads['b2']=db
        # dx,dw,db= Linear_ReLU.backward(dx,relu_back)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #   i=n+1
        #   self.params['W{}'.format(i)]=torch.zeros(out_dim,hidden_dim,dtype=dtype,device=device)
        #   self.params['W{}'.format(i)]+=weight_scale*torch.randn(out_dim,hidden_dim,dtype=dtype,device=device)
        #   self.params['b{}'.format(i)]=torch.zeros(hidden_dim,dtype=dtype,device=device)
        #   out_dim=hidden_dim

        # i+=1
        # self.params['W{}'.format(i)]=torch.zeros(out_dim,num_classes,dtype=dtype,device=device)
        # self.params['W{}'.format(i)]+=weight_scale*torch.randn(out_dim,num_classes,dtype=dtype,device=device)
    ...
    def loss(self, X, y=None):
        ...
        #   i=n+1
        #   input_forward,dict['cache_LR{}'.format(i)]=Linear_ReLU.forward(input_forward,self.params['W{}'.format(i)],self.params['b{}'.format(i)])
        #   if self.use_dropout:
        #     input_forward,dict['cache_Dropout{}'.format(i)]=Dropout.forward(input_forward,self.dropout_param)
        
        # i+=1
        # input_forward,dict['cache_L{}'.format(i)]=Linear.forward(input_forward,self.params['W{}'.format(i)],self.params['b{}'.format(i)])
        # scores=input_forward
        ...
        # last_dout,dw,db=Linear.backward(grad_softmax, dict['cache_L{}'.format(i)])
        # grads['W{}'.format(i)]=dw+2*self.params['W{}'.format(i)]*self.reg
        # grads['b{}'.format(i)]=db
        # for n  in range(self.num_layers-1)[::-1]:
        #   i=n+1
        #   if self.use_dropout:
        #     last_dout=Dropout.backward(last_dout,dict['cache_Dropout{}'.format(i)])
        #   last_dout,dw,db=Linear_ReLU.backward(last_dout, dict['cache_LR{}'.format(i)])
        #   grads['W{}'.format(i)]=dw+2*self.params['W{}'.format(i)]*self.reg
        #   grads['b{}'.format(i)]=db
        #   loss+=(self.params['W{}'.format(i)]*self.params['W{}'.format(i)]).sum()*self.reg
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict, lr_decay=0.9, optim_config={'learning_rate':1e-1}, device='cuda')
    solver.train()
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
