...
"""
import torch
from a4_helper import softmax_loss
...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        # # padding
        # x_copy=torch.zeros((N,C,H +2*pad,W+2*pad)).to(device=x.device,dtype=x.dtype)
        # x_copy[:,:,pad:-1-(pad-1),pad:-1-(pad-1)]=x.clone().to(device=x.device,dtype=x.dtype)

        # out=torch.zeros((N,F,H_out, W_out)).to(device=x.device, dtype=x.dtype)
        # idx_h,idx_w=0,0
        # for n in range(N):
        #   for f in range(F):
        #     idx_h=0
        ...
        #         out[n,f,h_out,w_out]=(x_copy[n,:,idx_h:idx_h+HH,idx_w:idx_w+WW]*w[f]).sum()+b[f]
        #         idx_w+=stride
        #       idx_h+=stride
        #####################################################################
        #                          END OF YOUR CODE                         #
    ...
    def backward(dout, cache):
        ...
        # dx=torch.zeros_like(x).to(device=dout.device,dtype=dout.dtype)
        # dw=torch.zeros_like(w).to(device=dout.device,dtype=dout.dtype)
        # db=dout.sum(dim=[0,2,3]).to(device=dout.device,dtype=dout.dtype) # filter dimension 남기고 합한다.

        # stride,pad=conv_param['stride'],conv_param['pad']
        # x_padded=torch.nn.functional.pad(x,(pad, pad, pad, pad)).to(device=dout.device,dtype=dout.dtype)
        # dx=torch.zeros_like(x_padded).to(device=dout.device,dtype=dout.dtype)

        # for n in range(N):
        #   for f in range(F):
        #     h_stride=0
        #     for H_dout in range(dout.shape[2]):
        #       w_stride=0
        #       for W_dout in range(dout.shape[3]):
        #         dx[n,:,h_stride:HH+h_stride,w_stride:WW +w_stride]+=w[f]*dout[n,f,H_dout,W_dout]
        #         dw[f]+= x_padded[n,:,h_stride:HH+h_stride,w_stride:WW +w_stride]*dout[n,f,H_dout,W_dout]
        #         w_stride+=stride
        #       h_stride+=stride
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out=torch.zeros((N,C,H_prime,W_prime)).to(device=x.device,dtype=x.dtype)
        for j in range(H_prime):
            for i in range(W_prime):
              out[:,:,j,i]=torch.max(torch.max(x[:,:,j*stride:j*stride+HH,i*stride:i*stride+WW],-1)[0],-1)[0]
        ...
        #         out[:,:,hh//stride,ww//stride]=torch.max(torch.max(
        #             x[:,:,hh:hh+pool_height,ww:ww+pool_width], dim=2).values, dim=2).values
        ...
        #       for c in range(C):
        #         out[n,c,h1,w1]=torch.max(x[n,c,h1*stride:h1*stride+pool_height,w1*stride:w1*stride+pool_width])
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache=(x,pool_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
            for c in range(C):
                for j in range(H_prime):
                    for i in range(W_prime):
                        ind=torch.argmax(x[n,c,j*stride:j*stride+HH, i*stride:i*stride+WW]) # max
                        ind1,ind2=ind//HH,ind%WW # maxpooling 했던 작은 칸에서의 index
                        dx[n,c,j*stride:j*stride+HH,i*stride:i*stride+WW][ind1,ind2]=dout[n,c,j,i] # 작은 칸에서의 index 위치에 dout을 더한다.
        ...
        # idx=0 #Index max value

        # for n in range(N):
        #   for c in range(C):
        #     for h1 in range(H_out):
        #       for w1 in range(W_out):
        #         idx=torch.argmax(x[n,c,h1*stride:h1*stride+pool_height,w1*stride:w1*stride+pool_width])

        #         # i, j: 2D  maximum value
        #         i=idx//pool_width
        #         j=((idx+1)%pool_width)-1

        #         dx[n,c,h1*stride:h1*stride+pool_height,w1*stride:w1*stride+pool_width][i,j]=dout[n,c,h1,w1]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for l in range(self.num_layers-1):
          ...
          if weight_scale=='kaiming':
            self.params['W'+str(l+1)]=kaiming_initializer(cat_flr[l],num_f, K=3,relu=True,device=device,dtype=dtype)
          else:
        ...
        # last layer size reduced
        
        if weight_scale=='kaiming':
          self.params["W"+str(self.num_layers)]=kaiming_initializer(final_input_dim,num_classes,relu=False,device=device,dtype=dtype)
        else:
    ...
    def loss(self, X, y=None):
        ...
        #last layer,only Linear forward
        ...
        # p_idx -> the end of max_pools
        # p_idx-=1
        for l in range(0,self.num_layers-1):
          #index 0 
          if self.num_layers-2-l in max_pool_set:
            #maxpooling, conv-relu-pool    
            if self.batchnorm:
              d_out,dw,db,dgamma,dbeta=Conv_BatchNorm_ReLU_Pool.backward(d_out,caches.pop())
            ...
            # no maxpool , conv-relu
            if self.batchnorm:
              d_out, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(d_out,caches.pop())
              grads['W'+str(self.num_layers-1-l)]=dw+2*self.reg*self.params['W'+str(self.num_layers-1-l)]
              grads['b'+str(self.num_layers-1-l)]=db
              grads['gamma'+str(self.num_layers-1-l)]=dgamma
              grads['beta'+str(self.num_layers-1-l)]=dbeta
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    input_dims=data_dict['X_train'].shape[1:]
    #input_dims=data_dict['X_train'].device
    ...
    solver = Solver(model, data_dict,
                    print_every=50, num_epochs=5, batch_size=150,
                    update_rule=adam,
                    optim_config={
                      'learning_rate': lr,
                    },
