...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        for image in range(N): # each image
          for flr in range(F): # each filter
            for i in range(H_out): # row
              for j in range(W_out): # colu
          ...
          for flr in range(F):
            ...
            for i in range(H_out):
              for j in range(W_out):
                dw[flr,:,:,:]+=dout[img,flr,i,j]*x_pad[img,:,i*stride:i*stride+HH,j*stride:j*stride+WW]
                dx[img,:,i*stride:i*stride+HH,j*stride:j*stride+WW]+=dout[img,flr,i,j]*w[flr,:,:,:]
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
          ...
          eps=cache['eps']
          dbeta=1.*dout.sum(dim=0)
          dgamma=torch.sum((dx-r_mean)/((r_val+eps)**0.5)*dout,dim=0)
